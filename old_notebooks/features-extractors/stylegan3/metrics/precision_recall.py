...
def compute_distances(row_features, col_features, num_gpus, rank, col_batch_size):
    ...
    dist_batches = []
    row_features = row_features.to(torch.float32) # added
    for col_batch in col_batches[rank :: num_gpus]:
        col_batch = col_batch.to(torch.float32)  # added
...
def compute_pr(opts, max_real, num_gen, nhood_size, row_batch_size, col_batch_size):
    ...
    # print("generated features:", gen_features)
    results = dict()
    for name, manifold, probes in [('precision', real_features, gen_features), ('recall', gen_features, real_features)]:
        ...
        for manifold_batch in manifold.split(row_batch_size):
            ...
            if dist.size(0) > nhood_size:  # Ensure there are enough distances to find the k-th value
                kth_value = dist.to(torch.float32).kthvalue(nhood_size + 1).values
            else:
                kth_value = torch.full((dist.size(0),), float('inf'), dtype=torch.float32)  # Handle edge case
            kth.append(kth_value.to(torch.float16) if opts.rank == 0 else None)        
